  ...
  useGSAP(
    () => {
      ...
      function startAnimation() {
        ...
        // --- ULTRA PREMIUM CINEMATIC TEXT ANIMATION ---
        const hasAnimated =
      ...
      function preloadImages() {
        let loadedCount = 0;
        const maxIndex =
          frameState.current.maxIndex;

        // 🚨 THE FIX: Force it to wait for ALL frames to load before starting
        const minFramesToStart =
          maxIndex;
        ...
        firstImg.onload = () => {
          images[0] = firstImg;
          renderFrame(0);
          onLoadProgress(1); // Set a tiny bit of progress
        };
        ...
        for (
          ...
          img.onload = () => {
            ...
            const percent = Math.min(
              100,
              (loadedCount / maxIndex) *
                100,
            );

            // Updates the global loader percentage accurately based on actual network speed
            onLoadProgress(percent);

            // Only triggers the entrance when loadedCount reaches 223
            if (
              !animationStarted &&
              loadedCount >=
                minFramesToStart
    ...
    >
      <div className="parent relative w-full h-[800vh] bg-gradient-to-b from-black via-neutral-900 to-black">
        ...
        <div className="sticky top-0 left-0 w-full h-screen z-10 flex items-center justify-center pointer-events-none overflow-hidden">
          ...
          >
            <span className="hero-word opacity-0">
              Build.
            </span>
            <span className="hero-word opacity-0">
              Innovate.
