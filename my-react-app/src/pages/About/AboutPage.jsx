...
// Director headshots stay in code, keyed by slug. Adding a new director
// in Sanity requires a matching slug entry here + a file under /public/directors/.
const DIRECTOR_IMAGES = {
  drmekapaparao:      "/directors/drmekapaparao.jpeg",
  hemantrao:          "/directors/hemantrao.png",
  mrsrajyalaskhmirao: "/directors/mrsrajyalaskhmirao.jpg",
};
...
export default function AboutPage() {
  ...
  }, { scope: pageRef, dependencies: [data.textRevealLines, data.stats, data.directors, data.valuesItems] });
        ...
        <div className="w-full max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-8 items-center z-10 relative">
          ...
          <div className="lg:col-span-7 flex flex-col z-20">
            ...
            <h1 className="text-[14vw] lg:text-[8.5vw] 2xl:text-[9rem] font-serif uppercase tracking-tighter leading-[0.95] text-[#050505] mix-blend-multiply">
              ...
              <span className="overflow-hidden block py-[0.2em] -my-[0.2em]">
                <span className="hero-title-word block">{loc(data.heroTitleWord1, lang)}</span>
              </span>
              ...
              <span className="overflow-hidden block py-[0.2em] -my-[0.2em] ms-[8vw] lg:ms-[4vw]">
                <span className="hero-title-word block text-black/25">{loc(data.heroTitleConnector, lang)}</span>
              </span>
              <span className="overflow-hidden block py-[0.2em] -my-[0.2em]">
                <span className="hero-title-word block">{loc(data.heroTitleWord2, lang)}</span>
              </span>
            </h1>
          ...
          <div className="lg:col-span-5 relative mt-12 lg:mt-0 lg:-ms-12 z-10">
            ...
            <div className="hero-accent-box absolute -bottom-8 -start-8 md:-start-12 bg-white p-6 md:p-8 shadow-2xl rounded-sm border border-black/5 max-w-[200px] md:max-w-[240px] z-20 hidden sm:block">
              <span className="text-[#0ea5a4] text-4xl md:text-5xl font-serif block mb-2 leading-none">{data.heroAccentNumber}</span>
              <span className="text-[10px] md:text-xs font-sans tracking-[0.2em] uppercase text-gray-500 font-bold leading-relaxed block">
                {loc(data.heroAccentLabel, lang)}
              </span>
            </div>
          </div>
      ...
      <section className="relative z-10 py-24 md:py-40 px-6 md:px-12 max-w-6xl mx-auto flex justify-start md:justify-end">
        <div className="w-full md:w-[85%]">
          <p className="text-3xl md:text-5xl lg:text-6xl font-serif leading-[1.3] md:leading-[1.2] tracking-tight">
            {data.textRevealLines.map((line, i) => (
              <span key={i} className="reveal-text text-black/10">{loc(line, lang)}</span>
            ))}
          </p>
        </div>
      </section>
      ...
      <section id="board" className="py-32 md:py-48 px-6 md:px-12 max-w-[1600px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 md:mb-28 gap-8">
          <div className="max-w-3xl">
            <span className="text-[#0ea5a4] text-xs font-sans tracking-[0.3em] uppercase mb-6 block font-bold">{loc(data.boardEyebrow, lang)}</span>
            <h2 className="text-5xl md:text-7xl lg:text-8xl font-serif uppercase tracking-tighter text-[#050505]">{loc(data.boardTitle, lang)}</h2>
          </div>
          <p className="max-w-sm text-gray-600 font-sans text-base leading-relaxed md:pb-3">
            {loc(data.boardDescription, lang)}
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 lg:gap-12">
          {data.directors.map((person) => (
            <div key={person.slug} className="group cursor-pointer">
              <div className="relative w-full aspect-[4/5] overflow-hidden bg-zinc-200 mb-8 rounded-sm shadow-xl">
                <img
                  src={DIRECTOR_IMAGES[person.slug] || ""}
                  alt={person.name}
                  loading="lazy"
                  decoding="async"
